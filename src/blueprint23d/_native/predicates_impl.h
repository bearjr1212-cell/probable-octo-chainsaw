 ...
 *   1997, pp. 305-363.
 *
 * Shared by predicates.c (Python bindings) and cdt.c (triangulation).
 *
...
#ifndef BLUEPRINT23D_PREDICATES_IMPL_H
#define BLUEPRINT23D_PREDICATES_IMPL_H
...
/*
 * Filtered + exact in-circle sign, for direct use by other kernels.
 * Positive means pd lies inside the circle through pa, pb, pc (given
 * counter-clockwise); zero means exactly cocircular.
 */
static int incircle_sign(const REAL *pa, const REAL *pb, const REAL *pc, const REAL *pd)
{
    ...
    if ((det > errbound) || (-det > errbound)) {
        return sign_of_real(det);
    }

    stat_incircle_defer++;
    return incircle_exact_sign(pa, pb, pc, pd);
}

/* Exact orientation sign: +1 counter-clockwise, -1 clockwise, 0 collinear. */
static int orient2d_sign(const REAL *pa, const REAL *pb, const REAL *pc)
{
    return sign_of_real(orient2d_value(pa, pb, pc));
}

#endif /* BLUEPRINT23D_PREDICATES_IMPL_H */
